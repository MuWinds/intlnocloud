...
if(file_exists('../Core/Config.php') && $do < 4){
	$installed=true;
	$do='0';
}
...
<div class="panel panel-primary">
	<div class="panel-heading" >
		<h3 class="panel-title" align="center">INTL即时到账</h3>
	</div>
	<div class="panel-body">
		<p><iframe src="../readme.txt" style="width:100%;height:465px;"></iframe></p>
		<?php if($installed){ ?>
		<div class="alert alert-warning">您已经安装过，如需重新安装请删除<font color=red> Core/Config.php </font>文件后再安装！</div>
		<?php }else{?>
...
include_once '../Core/Config.php';
	if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
		...
		if(!$con=DB::connect($dbconfig['host'],$dbconfig['user'],$dbconfig['pwd'],$dbconfig['dbname'],$dbconfig['port'])){
			...
			if(DB::query("select * from pay_config where 1")==FALSE)
				echo '<p align="right"><a class="btn btn-primary btn-block" href="?do=4">创建数据表>></a></p>';
			else
				echo '<div class="list-group-item list-group-item-info">系统检测到你此系统</div>
...
?>";
		if(!$con=DB::connect($db_host,$db_user,$db_pwd,$db_name,$db_port)){
			...
			if(DB::query("select * from shua_config where 1")==FALSE)
				...
				echo '<div class="list-group-item list-group-item-info">系统检测到此系统</div>
				<div class="list-group-item">
					<a href="?do=6" class="btn btn-block btn-info">跳过安装</a>
				</div>
				<div class="list-group-item">
...
if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
	...
	DB::query("INSERT INTO `pay_config` VALUES ('build', '".$date."')");
	$MD5_SKY = MD5(date("YmdHis"));
	DB::query("INSERT INTO `pay_config` VALUES ('SKY', '".$MD5_SKY."')");
}
if($e==0) {
...
<?php
	...
	echo '<div class="alert alert-info"><font color="green">安装完成！管理账号和密码是:admin/123456</font><br/><br/><a href="../">>>网站首页</a>｜<a href="../Admin/">>>后台管理</a><hr/>更多设置选项请登录后台管理进行修改。<br/><br/><font color="#FF0033">如果你的空间不支持本地文件读写，请自行在Core/ 目录建立 Config.php 文件！</font></div>';
?>
	</div>
</div>
