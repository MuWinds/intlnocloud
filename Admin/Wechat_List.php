...
<form onsubmit="return searchOrder()" method="GET" class="form-inline">
  ...
  <a href="javascript:listTable('start')" class="btn btn-default" title="刷新"><i class="fa fa-refresh"></i></a>
  <div class="form-group">
	<select id="dstatus" class="form-control"><option value="0">显示全部</option><option value="1">只显示已登录</option><option value="2">只显示未登录</option></select>
  </div>
</form>
...
function listTable(query){
	...
	if(query == 'start' || query == undefined){
		query = '';
		history.replaceState({}, null, './Wechat_List.php');
	}else if(query != undefined){
		history.replaceState({}, null, './Wechat_List.php?'+query);
	}
	layer.closeAll();
	var ii = layer.load(2, {shade:[0.1,'#fff']});
	$.ajax({
		type : 'GET',
		url : 'wechat_List-table.php?dstatus='+dstatus+'&'+query,
		dataType : 'html',
		cache : false,
...
function WesetStatus(id, status) {
	if(status==5){
		var confirmobj = layer.confirm('你确实要删除此微信吗？', {
			btn: ['确定','取消']
		}, function(){
...
function setStatusDo(id, status) {
	...
	$.ajax({
		type : 'get',
		url : 'Ajax.php',
		data : 'act=WesetStatus&id=' + id + '&status=' + status,
		dataType : 'json',
